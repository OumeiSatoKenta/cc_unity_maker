...
public static class Setup032_SpinCutter
{
    ...
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup032_SpinCutter] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.08f, 0.05f, 0.12f, 1f); camera.orthographic = true; camera.orthographicSize = 5f; }

        string whiteTexPath = "Assets/Scripts/Game032_SpinCutter/WhiteSquare.png";
        ...
        string sp = "Assets/Resources/Sprites/Game032_SpinCutter/"; string pd = "Assets/Scripts/Game032_SpinCutter/";

        var bladePrefab = SP(pd + "BladePrefab.prefab", AssetDatabase.LoadAssetAtPath<Sprite>(sp + "blade.png"), whiteSprite, 10);
        var enemyPrefab = SP(pd + "EnemyPrefab.prefab", AssetDatabase.LoadAssetAtPath<Sprite>(sp + "enemy.png"), whiteSprite, 5);
        var playerPrefab = SP(pd + "PlayerPrefab.prefab", AssetDatabase.LoadAssetAtPath<Sprite>(sp + "player.png"), whiteSprite, 8);

        var gmObj = new GameObject("GameManager"); var gm = gmObj.AddComponent<SpinCutterGameManager>();
        var boardObj = new GameObject("SpinBoard"); boardObj.transform.SetParent(gmObj.transform);
        var sm = boardObj.AddComponent<SpinManager>();
        var smSO = new SerializedObject(sm);
        smSO.FindProperty("_bladePrefab").objectReferenceValue = bladePrefab;
        smSO.FindProperty("_enemyPrefab").objectReferenceValue = enemyPrefab;
        smSO.FindProperty("_playerPrefab").objectReferenceValue = playerPrefab;
        smSO.FindProperty("_bladeOrbitRadius").floatValue = 2f;
        smSO.FindProperty("_bladeSpeed").floatValue = 300f;
        smSO.FindProperty("_spawnInterval").floatValue = 2f;
        smSO.FindProperty("_gameTime").floatValue = 30f;
        ...
        var retryBtn = CB(resultPanel.transform, "RetryButton", "リトライ", 28, jpFont, new Vector2(0.5f,0.15f), new Vector2(0.5f,0.15f), new Vector2(0.5f,0.5f), new Vector2(200,60), Vector2.zero, new Color(0.5f,0.2f,0.5f,1f));
        resultPanel.SetActive(false);

        var uiObj = new GameObject("SpinCutterUI"); var scUI = uiObj.AddComponent<SpinCutterUI>();
        ...
        gmSO.FindProperty("_spinManager").objectReferenceValue = sm;
        ...
        Debug.Log("[Setup032_SpinCutter] SpinCutter シーンを作成しました: " + scenePath);
    }
