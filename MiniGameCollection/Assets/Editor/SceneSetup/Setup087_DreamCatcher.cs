...
using TMPro;
using System.IO;
using Game087_DreamCatcher;

public static class Setup087_DreamCatcher
{
    [MenuItem("Assets/Setup/087 DreamCatcher")]
    public static void CreateScene()
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup087] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game087_DreamCatcher/";

        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.08f, 0.04f, 0.2f); camera.orthographic = true; camera.orthographicSize = 6f; }
        ...
        var gm = gmObj.AddComponent<DreamCatcherGameManager>();

        var dmObj = new GameObject("DreamManager"); dmObj.transform.SetParent(gmObj.transform);
        var dm = dmObj.AddComponent<DreamManager>();
        var dmSO = new SerializedObject(dm);
        dmSO.FindProperty("_fragmentSprite").objectReferenceValue = fragmentSprite;
        dmSO.FindProperty("_catcherSprite").objectReferenceValue = catcherSprite;
        dmSO.FindProperty("_spawnInterval").floatValue = 2f;
        ...
        var collectionText = CT(canvasObj.transform, "CollectionText", "夢: 0/6種", 32, jpFont, new Vector2(0,1), new Vector2(0,1), new Vector2(0,1), new Vector2(200,40), new Vector2(20,-15));
        collectionText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Left; collectionText.GetComponent<TextMeshProUGUI>().color = new Color(0.8f,0.7f,1f);

        var fragmentText = CT(canvasObj.transform, "FragmentText", "破片: 0", 28, jpFont, new Vector2(1,1), new Vector2(1,1), new Vector2(1,1), new Vector2(150,35), new Vector2(-20,-15));
        fragmentText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Right; fragmentText.GetComponent<TextMeshProUGUI>().color = new Color(1f,0.9f,0.7f);

        var menuBtn = CB(canvasObj.transform, "MenuButton", "メニューへ戻る", 26, jpFont, new Vector2(0,0), new Vector2(0,0), new Vector2(0,0), new Vector2(260,55), new Vector2(20,20), new Color(0.2f,0.15f,0.3f,0.9f));
        menuBtn.AddComponent<BackToMenuButton>();

        var clearPanel = CreatePanel(canvasObj.transform, "ClearPanel", new Color(0.1f,0.06f,0.25f,0.95f));
        CT(clearPanel.transform, "CT", "夢の世界完成！", 48, jpFont, new Vector2(0.5f,0.75f), new Vector2(0.5f,0.75f), new Vector2(0.5f,0.5f), new Vector2(600,80), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        ...
        var uiObj = new GameObject("DreamCatcherUI"); uiObj.transform.SetParent(gmObj.transform);
        var ui = uiObj.AddComponent<DreamCatcherUI>();
        var uiSO = new SerializedObject(ui);
        uiSO.FindProperty("_collectionText").objectReferenceValue = collectionText.GetComponent<TextMeshProUGUI>();
        uiSO.FindProperty("_fragmentText").objectReferenceValue = fragmentText.GetComponent<TextMeshProUGUI>();
        ...
        gmSO.FindProperty("_dreamManager").objectReferenceValue = dm;
        gmSO.FindProperty("_ui").objectReferenceValue = ui;
        gmSO.FindProperty("_totalFragmentTypes").intValue = 6;
        ...
        string scenePath = "Assets/Scenes/087_DreamCatcher.unity";
        EditorSceneManager.SaveScene(scene, scenePath);
        AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup087] DreamCatcher シーンを作成しました: " + scenePath);
    }
