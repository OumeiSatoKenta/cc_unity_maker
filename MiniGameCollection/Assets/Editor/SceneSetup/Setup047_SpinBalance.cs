...
public static class Setup047_SpinBalance
{
    ...
    public static void CreateScene()
    {
        ...
        string sp = "Assets/Resources/Sprites/Game047_SpinBalance/";

        // Camera setup
        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.78f, 0.85f, 0.92f); camera.orthographic = true; camera.orthographicSize = 6f; }

        // Load sprites
        Sprite bgSprite = LoadSprite(sp + "background.png");
        Sprite boardSprite = LoadSprite(sp + "board.png");
        Sprite pieceSprite = LoadSprite(sp + "piece.png");
        ...
        boardCol.size = new Vector2(5f, 1f);

        // FallZone (trigger at bottom of screen)
        var fallZoneObj = new GameObject("FallZone");
        fallZoneObj.transform.position = new Vector3(0f, -7f, 0f);
        var fallCol = fallZoneObj.AddComponent<BoxCollider2D>();
        fallCol.size = new Vector2(20f, 2f);
        ...
        var bm = bmObj.AddComponent<BalanceManager>();

        // Wire BalanceManager
        var bmSO = new SerializedObject(bm);
        bmSO.FindProperty("_gameManager").objectReferenceValue = gm;
        bmSO.FindProperty("_board").objectReferenceValue = boardObj.transform;
        bmSO.FindProperty("_pieceSprite").objectReferenceValue = pieceSprite;
        bmSO.FindProperty("_initialPieceCount").intValue = 3;
        bmSO.FindProperty("_maxPieceCount").intValue = 8;
        bmSO.FindProperty("_spawnInterval").floatValue = 3f;
        bmSO.FindProperty("_rotationSensitivity").floatValue = 0.3f;
        ...
        var timerText = CT(canvasObj.transform, "TimerText", "30.0s", 36, jpFont,
            new Vector2(1, 1), new Vector2(1, 1), new Vector2(1, 1), new Vector2(200, 50), new Vector2(-20, -20));
        ...
        // Piece count text (top left)
        var pieceCountText = CT(canvasObj.transform, "PieceCountText", "コマ: 3", 36, jpFont,
            new Vector2(0, 1), new Vector2(0, 1), new Vector2(0, 1), new Vector2(200, 50), new Vector2(20, -20));
        pieceCountText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Left;
        pieceCountText.GetComponent<TextMeshProUGUI>().color = Color.black;
        ...
        var clearScoreText = CT(clearPanel.transform, "ClearScore", "", 40, jpFont,
        ...
        uiSO.FindProperty("_pieceCountText").objectReferenceValue = pieceCountText.GetComponent<TextMeshProUGUI>();
        ...
        gmSO.FindProperty("_balanceManager").objectReferenceValue = bm;
        ...
        // Save scene
        string scenePath = "Assets/Scenes/047_SpinBalance.unity";
        EditorSceneManager.SaveScene(scene, scenePath);
        AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup047] SpinBalance シーンを作成しました: " + scenePath);
    }
