using UnityEngine;using UnityEngine.UI;using UnityEditor;using UnityEditor.SceneManagement;using UnityEngine.InputSystem.UI;using TMPro;using Game056_InflateFloat;
public static class Setup056_InflateFloat
{
    ...
    public static void CreateScene()
    {
        ...
        var camera=Object.FindFirstObjectByType<Camera>();if(camera!=null){camera.backgroundColor=new Color(0.55f,0.75f,0.95f,1f);camera.orthographic=true;camera.orthographicSize=5f;}

        var gmObj=new GameObject("GameManager");var gm=gmObj.AddComponent<InflateFloatGameManager>();
        var boardObj=new GameObject("BalloonBoard");boardObj.transform.SetParent(gmObj.transform);var bm=boardObj.AddComponent<BalloonManager>();
        ...
        var hint=CT(canvasObj.transform,"HintText","長押しで膨らむ!左右で移動!",18,jpFont,new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(450,35),new Vector2(0,20));
        ...
        var menuBtn=CB(canvasObj.transform,"MenuButton","メニューへ戻る",24,jpFont,new Vector2(1,1),new Vector2(1,1),new Vector2(1,1),new Vector2(240,50),new Vector2(-20,-20),new Color(0.3f,0.4f,0.6f,0.9f));
        ...
        CT(clrPanel.transform,"T","クリア!",48,jpFont,new Vector2(0.5f,0.65f),new Vector2(0.5f,0.65f),new Vector2(0.5f,0.5f),new Vector2(300,60),Vector2.zero).GetComponent<TextMeshProUGUI>().alignment=TextAlignmentOptions.Center;
        var nextBtn=CB(clrPanel.transform,"NextBtn","次のステージ",28,jpFont,new Vector2(0.5f,0.2f),new Vector2(0.5f,0.2f),new Vector2(0.5f,0.5f),new Vector2(220,60),Vector2.zero,new Color(0.2f,0.6f,0.3f,1f));
        ...
        var failText=CT(failPanel.transform,"FT","失敗!",48,jpFont,new Vector2(0.5f,0.65f),new Vector2(0.5f,0.65f),new Vector2(0.5f,0.5f),new Vector2(300,60),Vector2.zero);
        failText.GetComponent<TextMeshProUGUI>().alignment=TextAlignmentOptions.Center;
        var retryBtn=CB(failPanel.transform,"RetryBtn","リトライ",28,jpFont,new Vector2(0.5f,0.2f),new Vector2(0.5f,0.2f),new Vector2(0.5f,0.5f),new Vector2(200,60),Vector2.zero,new Color(0.7f,0.3f,0.2f,1f));
        failPanel.SetActive(false);

        var uiObj=new GameObject("InflateFloatUI");var ifUI=uiObj.AddComponent<InflateFloatUI>();
        var uiSO=new SerializedObject(ifUI);uiSO.FindProperty("_stageText").objectReferenceValue=stageText.GetComponent<TextMeshProUGUI>();
        uiSO.FindProperty("_clearPanel").objectReferenceValue=clrPanel;uiSO.FindProperty("_failPanel").objectReferenceValue=failPanel;
        uiSO.FindProperty("_failReasonText").objectReferenceValue=failText.GetComponent<TextMeshProUGUI>();
        uiSO.FindProperty("_gameManager").objectReferenceValue=gm;uiSO.ApplyModifiedProperties();

        menuBtn.AddComponent<BackToMenuButton>();var gmSO=new SerializedObject(gm);gmSO.FindProperty("_balloonManager").objectReferenceValue=bm;gmSO.FindProperty("_ui").objectReferenceValue=ifUI;gmSO.ApplyModifiedProperties();
        nextBtn.GetComponent<Button>().onClick.AddListener(ifUI.OnNextButton);
        retryBtn.GetComponent<Button>().onClick.AddListener(ifUI.OnRetryButton);
        ...
        string scenePath="Assets/Scenes/056_InflateFloat.unity";EditorSceneManager.SaveScene(scene,scenePath);AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup056_InflateFloat] シーンを作成しました: "+scenePath);
    }
