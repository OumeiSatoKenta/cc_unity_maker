...
using TMPro;
using System.IO;
using Game100v2_DreamRun;

public static class Setup100v2_DreamRun
{
    [MenuItem("Assets/Setup/100v2 DreamRun")]
    public static void CreateScene()
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup100v2] Play モード中は実行できません。"); return; }
        EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);

        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game100v2_DreamRun/";
        ...
        if (camera != null)
        {
            ...
            camera.backgroundColor = new Color(0.06f, 0.02f, 0.16f);
        ...
        Sprite runnerJumpSprite = LoadSprite(sp + "runner_jump.png");
        Sprite obstacleGroundSprite = LoadSprite(sp + "obstacle_ground.png");
        Sprite obstacleAirSprite = LoadSprite(sp + "obstacle_air.png");
        Sprite fragmentSprite = LoadSprite(sp + "fragment.png");
        Sprite backgroundSprite = LoadSprite(sp + "background.png");
        Sprite bgLayerSprite = LoadSprite(sp + "background_layer1.png");
        Sprite gravityZoneSprite = LoadSprite(sp + "gravity_zone.png");

        // === GameManager hierarchy ===
        var gmObj = new GameObject("DreamRunGameManager");
        ...
        {
            new StageManager.StageConfig { speedMultiplier = 1.0f, countMultiplier = 1, complexityFactor = 0.0f, stageName = "Stage 1", customData = "2,5,0.8,false,false" },
            new StageManager.StageConfig { speedMultiplier = 1.5f, countMultiplier = 1, complexityFactor = 0.3f, stageName = "Stage 2", customData = "3,7,0.7,false,false" },
            new StageManager.StageConfig { speedMultiplier = 2.0f, countMultiplier = 1, complexityFactor = 0.5f, stageName = "Stage 3", customData = "3,8,0.6,true,false" },
            new StageManager.StageConfig { speedMultiplier = 2.5f, countMultiplier = 1, complexityFactor = 0.7f, stageName = "Stage 4", customData = "3,10,0.5,true,true" },
            new StageManager.StageConfig { speedMultiplier = 3.0f, countMultiplier = 1, complexityFactor = 1.0f, stageName = "Stage 5", customData = "3,12,0.4,true,true" },
        };
        sm.SetConfigs(stageConfigs);

        // DreamRunManager (child of GM)
        var dmObj = new GameObject("DreamRunManager");
        dmObj.transform.SetParent(gmObj.transform);
        var dm = dmObj.AddComponent<DreamRunManager>();
        SetPrivateField(dm, "_gameManager", gm);
        SetPrivateField(dm, "_runnerSprite", runnerSprite);
        SetPrivateField(dm, "_runnerJumpSprite", runnerJumpSprite);
        SetPrivateField(dm, "_obstacleGroundSprite", obstacleGroundSprite);
        SetPrivateField(dm, "_obstacleAirSprite", obstacleAirSprite);
        SetPrivateField(dm, "_fragmentSprite", fragmentSprite);
        SetPrivateField(dm, "_backgroundSprite", backgroundSprite);
        SetPrivateField(dm, "_bgLayerSprite", bgLayerSprite);
        SetPrivateField(dm, "_gravityZoneSprite", gravityZoneSprite);
        ...
        var livesTextGo = CT(canvasObj.transform, "LivesText", "♥♥♥", 48, jpFont,
            new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f),
            new Vector2(300, 60), new Vector2(0, -15));
        livesTextGo.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        livesTextGo.GetComponent<TextMeshProUGUI>().color = new Color(1f, 0.3f, 0.4f);

        var fragmentTextGo = CT(canvasObj.transform, "FragmentText", "断片 0/5", 36, jpFont,
            new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f),
            new Vector2(300, 50), new Vector2(0, -75));
        fragmentTextGo.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        fragmentTextGo.GetComponent<TextMeshProUGUI>().color = new Color(1f, 0.9f, 0.3f);
        ...
        var goTitleGo = CT(goPanel.transform, "GameOverText", "ゲームオーバー", 52, jpFont,
            new Vector2(0.5f, 0.75f), new Vector2(0.5f, 0.75f), new Vector2(0.5f, 0.5f),
            new Vector2(620, 200), Vector2.zero);
        ...
        // === DreamRunUI ===
        var uiGo = new GameObject("DreamRunUI");
        uiGo.transform.SetParent(canvasObj.transform, false);
        var ui = uiGo.AddComponent<DreamRunUI>();
        ...
        SetPrivateField(ui, "_livesText", livesTextGo.GetComponent<TextMeshProUGUI>());
        SetPrivateField(ui, "_fragmentText", fragmentTextGo.GetComponent<TextMeshProUGUI>());
        ...
        var ipTitleGo = CT(ipPanelGo.transform, "IPTitle", "DreamRun", 72, jpFont,
        ...
        ipTitleGo.GetComponent<TextMeshProUGUI>().color = new Color(0.76f, 1f, 0.3f);

        var ipDescGo = CT(ipPanelGo.transform, "IPDescription", "夢の世界を走り抜けよう", 48, jpFont,
        ...
        var ipControlsGo = CT(ipPanelGo.transform, "IPControls", "画面左タップでレーン左移動、中央タップでジャンプ、右タップでレーン右移動", 36, jpFont,
            new Vector2(0.5f, 0.43f), new Vector2(0.5f, 0.43f), new Vector2(0.5f, 0.5f),
            new Vector2(920, 90), Vector2.zero);
        ...
        ipControlsGo.GetComponent<TextMeshProUGUI>().enableWordWrapping = true;

        var ipGoalGo = CT(ipPanelGo.transform, "IPGoal", "夢の断片をすべて集めてストーリーを完成させよう", 40, jpFont,
        ...
        var ipBtnGo = CB(ipPanelGo.transform, "IPStartButton", "はじめる", 52, jpFont,
            ...
            new Vector2(420, 90), Vector2.zero, new Color(0.2f, 0.6f, 0.1f));
        ...
        SetPrivateField(gm, "_dreamRunManager", dm);
        SetPrivateField(gm, "_ui", ui);

        // === Save scene ===
        string scenePath = "Assets/Scenes/100v2_DreamRun.unity";
        EditorSceneManager.SaveScene(UnityEngine.SceneManagement.SceneManager.GetActiveScene(), scenePath);
        AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup100v2] DreamRun scene created: " + scenePath);
    }
    ...
    static Sprite LoadSprite(string path)
    {
        ...
        if (!File.Exists(path)) { Debug.LogWarning("[Setup100v2] Sprite not found: " + path); return null; }
    ...
    static void SetPrivateField(object obj, string fieldName, object value)
    {
        ...
        else Debug.LogWarning($"[Setup100v2] Field '{fieldName}' not found on {obj.GetType().Name}");
    }
}
