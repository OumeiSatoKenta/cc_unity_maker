using UnityEngine;using UnityEngine.UI;using UnityEditor;using UnityEditor.SceneManagement;using UnityEngine.InputSystem.UI;using TMPro;using Game039_BoomerangHero;
public static class Setup039_BoomerangHero
{
    [MenuItem("Assets/Setup/039 BoomerangHero")]
    public static void CreateScene()
    {
        if(EditorApplication.isPlaying){Debug.LogError("[Setup039_BoomerangHero] Play モード中は実行できません。");return;}
        ...
        var camera=Object.FindFirstObjectByType<Camera>();if(camera!=null){camera.backgroundColor=new Color(0.15f,0.2f,0.1f,1f);camera.orthographic=true;camera.orthographicSize=5f;}
        string whiteTexPath="Assets/Scripts/Game039_BoomerangHero/WhiteSquare.png";
        ...
        var whiteSprite=AssetDatabase.LoadAssetAtPath<Sprite>(whiteTexPath);string sp="Assets/Resources/Sprites/Game039_BoomerangHero/";string pd="Assets/Scripts/Game039_BoomerangHero/";
        var boomerangPrefab=SP(pd+"BoomerangPrefab.prefab",AssetDatabase.LoadAssetAtPath<Sprite>(sp+"boomerang.png"),whiteSprite,10);
        var heroPrefab=SP(pd+"HeroPrefab.prefab",AssetDatabase.LoadAssetAtPath<Sprite>(sp+"hero.png"),whiteSprite,8);
        var enemyPrefab=SP(pd+"EnemyPrefab.prefab",AssetDatabase.LoadAssetAtPath<Sprite>(sp+"enemy.png"),whiteSprite,5);

        var gmObj=new GameObject("GameManager");var gm=gmObj.AddComponent<BoomerangHeroGameManager>();
        var boardObj=new GameObject("BoomerangBoard");boardObj.transform.SetParent(gmObj.transform);var bm=boardObj.AddComponent<BoomerangManager>();
        var bmSO=new SerializedObject(bm);bmSO.FindProperty("_boomerangPrefab").objectReferenceValue=boomerangPrefab;bmSO.FindProperty("_heroPrefab").objectReferenceValue=heroPrefab;
        bmSO.FindProperty("_enemyPrefab").objectReferenceValue=enemyPrefab;bmSO.FindProperty("_boomerangSpeed").floatValue=8f;bmSO.FindProperty("_spawnInterval").floatValue=2f;
        bmSO.FindProperty("_gameTime").floatValue=30f;bmSO.FindProperty("_maxEnemies").intValue=6;bmSO.ApplyModifiedProperties();
        ...
        var hint=CT(canvasObj.transform,"HintText","ドラッグでブーメランの軌道を描こう!",20,jpFont,new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(500,40),new Vector2(0,20));
        hint.GetComponent<TextMeshProUGUI>().alignment=TextAlignmentOptions.Center;hint.GetComponent<TextMeshProUGUI>().color=new Color(0.5f,0.6f,0.4f);
        ...
        var retryBtn=CB(resultPanel.transform,"RetryButton","リトライ",28,jpFont,new Vector2(0.5f,0.15f),new Vector2(0.5f,0.15f),new Vector2(0.5f,0.5f),new Vector2(200,60),Vector2.zero,new Color(0.4f,0.6f,0.3f,1f));
        resultPanel.SetActive(false);

        var uiObj=new GameObject("BoomerangHeroUI");var bhUI=uiObj.AddComponent<BoomerangHeroUI>();
        var uiSO=new SerializedObject(bhUI);uiSO.FindProperty("_scoreText").objectReferenceValue=scoreText.GetComponent<TextMeshProUGUI>();
        ...
        menuBtn.AddComponent<BackToMenuButton>();var gmSO=new SerializedObject(gm);gmSO.FindProperty("_boomerangManager").objectReferenceValue=bm;gmSO.FindProperty("_ui").objectReferenceValue=bhUI;gmSO.ApplyModifiedProperties();
        ...
        string scenePath="Assets/Scenes/039_BoomerangHero.unity";EditorSceneManager.SaveScene(scene,scenePath);AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup039_BoomerangHero] BoomerangHero シーンを作成しました: "+scenePath);
    }
