...
public static class Setup040_DashDungeon
{
    ...
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup040] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game040_DashDungeon/";

        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.08f, 0.06f, 0.12f); camera.orthographic = true; camera.orthographicSize = 5.5f; }

        Sprite bgSprite = LoadSprite(sp + "background.png");
        Sprite heroSprite = LoadSprite(sp + "hero.png");
        ...
        var bgObj = new GameObject("Background"); bgObj.AddComponent<SpriteRenderer>().sprite = bgSprite; bgObj.GetComponent<SpriteRenderer>().sortingOrder = -10;
        ...
        var dmObj = new GameObject("DungeonManager"); dmObj.transform.SetParent(gmObj.transform);
        var dm = dmObj.AddComponent<DungeonManager>();
        var dmSO = new SerializedObject(dm);
        dmSO.FindProperty("_gameManager").objectReferenceValue = gm;
        dmSO.FindProperty("_heroSprite").objectReferenceValue = heroSprite;
        dmSO.FindProperty("_enemySprite").objectReferenceValue = enemySprite;
        dmSO.FindProperty("_wallSprite").objectReferenceValue = wallSprite;
        dmSO.FindProperty("_exitSprite").objectReferenceValue = exitSprite;
        ...
        var hpText = CT(canvasObj.transform, "HpText", "HP: 5/5", 36, jpFont, new Vector2(0,1), new Vector2(0,1), new Vector2(0,1), new Vector2(200,50), new Vector2(20,-20));
        hpText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Left;
        var scoreText = CT(canvasObj.transform, "ScoreText", "Score: 0", 36, jpFont, new Vector2(1,1), new Vector2(1,1), new Vector2(1,1), new Vector2(250,50), new Vector2(-20,-20));
        ...
        var clearPanel = CreatePanel(canvasObj.transform, "ClearPanel", new Color(0.1f,0.3f,0.1f,0.9f));
        CT(clearPanel.transform, "CT", "ダンジョンクリア！", 48, jpFont, new Vector2(0.5f,0.75f), new Vector2(0.5f,0.75f), new Vector2(0.5f,0.5f), new Vector2(600,100), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        ...
        CT(goPanel.transform, "GT", "力尽きた...", 52, jpFont, new Vector2(0.5f,0.75f), new Vector2(0.5f,0.75f), new Vector2(0.5f,0.5f), new Vector2(600,100), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        ...
        var uiObj = new GameObject("DashDungeonUI"); uiObj.transform.SetParent(gmObj.transform);
        var ui = uiObj.AddComponent<DashDungeonUI>();
        ...
        gmSO.FindProperty("_maxHp").intValue = 5;
        ...
        string scenePath = "Assets/Scenes/040_DashDungeon.unity";
        EditorSceneManager.SaveScene(scene, scenePath); AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup040] DashDungeon シーンを作成しました: " + scenePath);
    }
