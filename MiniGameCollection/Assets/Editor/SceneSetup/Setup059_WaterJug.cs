...
using TMPro;
using System.IO;
using Game059_WaterJug;

public static class Setup059_WaterJug
{
    [MenuItem("Assets/Setup/059 WaterJug")]
    public static void CreateScene()
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup059] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game059_WaterJug/";
        ...
        Sprite waterSprite = LoadSprite(sp + "water.png");
        ...
        var jmObj = new GameObject("JugManager"); jmObj.transform.SetParent(gmObj.transform);
        var jm = jmObj.AddComponent<JugManager>();
        var jmSO = new SerializedObject(jm);
        jmSO.FindProperty("_gameManager").objectReferenceValue = gm;
        jmSO.FindProperty("_jugSprite").objectReferenceValue = jugSprite;
        jmSO.FindProperty("_waterSprite").objectReferenceValue = waterSprite;
        jmSO.ApplyModifiedProperties();
        ...
        var movesText = CT(canvasObj.transform, "MovesText", "残り: 15手", 36, jpFont, new Vector2(1,1), new Vector2(1,1), new Vector2(1,1), new Vector2(200,50), new Vector2(-20,-20));
        movesText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Right;
        movesText.GetComponent<TextMeshProUGUI>().color = Color.white;

        var stateText = CT(canvasObj.transform, "StateText", "", 36, jpFont, new Vector2(0.5f,1), new Vector2(0.5f,1), new Vector2(0.5f,1), new Vector2(400,50), new Vector2(0,-20));
        stateText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        stateText.GetComponent<TextMeshProUGUI>().color = Color.white;
        ...
        var uiObj = new GameObject("WaterJugUI"); uiObj.transform.SetParent(gmObj.transform);
        ...
        uiSO.FindProperty("_stateText").objectReferenceValue = stateText.GetComponent<TextMeshProUGUI>();
        ...
        gmSO.FindProperty("_jugManager").objectReferenceValue = jm;
        gmSO.FindProperty("_ui").objectReferenceValue = ui;
        gmSO.FindProperty("_maxMoves").intValue = 15;
        ...
        string scenePath = "Assets/Scenes/059_WaterJug.unity";
        EditorSceneManager.SaveScene(scene, scenePath);
        AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup059] WaterJug シーンを作成しました: " + scenePath);
    }
