...
public static class Setup055_DustSweep
{
    ...
    public static void CreateScene()
    {
        ...
        string sp = "Assets/Resources/Sprites/Game055_DustSweep/";

        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.86f, 0.82f, 0.74f); camera.orthographic = true; camera.orthographicSize = 5.5f; }

        Sprite bgSprite = LoadSprite(sp + "background.png");
        Sprite dustSprite = LoadSprite(sp + "dust.png");
        Sprite hiddenSprite = LoadSprite(sp + "hidden.png");
        ...
        var hiddenObj = new GameObject("HiddenPicture");
        hiddenObj.transform.position = Vector3.zero;
        var hiddenSr = hiddenObj.AddComponent<SpriteRenderer>(); hiddenSr.sprite = hiddenSprite; hiddenSr.sortingOrder = 1;
        hiddenObj.transform.localScale = new Vector3(0.03f, 0.03f, 1f);

        var gmObj = new GameObject("GameManager");
        var gm = gmObj.AddComponent<DustSweepGameManager>();

        var smObj = new GameObject("SweepManager"); smObj.transform.SetParent(gmObj.transform);
        var sm = smObj.AddComponent<SweepManager>();
        var smSO = new SerializedObject(sm);
        smSO.FindProperty("_dustSprite").objectReferenceValue = dustSprite;
        smSO.FindProperty("_gridWidth").intValue = 8;
        smSO.FindProperty("_gridHeight").intValue = 10;
        smSO.FindProperty("_brushRadius").floatValue = 0.8f;
        ...
        // Progress slider
        var progressSlider = CreateSlider(canvasObj.transform, "ProgressSlider", jpFont, "清潔度",
            new Vector2(0,1), new Vector2(0,1), new Vector2(0,1), new Vector2(300,35), new Vector2(20,-20), new Color(0.3f,0.7f,0.3f));
        ...
        CT(clearPanel.transform, "CT", "ピカピカ！", 52, jpFont, new Vector2(0.5f,0.75f), new Vector2(0.5f,0.75f), new Vector2(0.5f,0.5f), new Vector2(600,100), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        ...
        var goProgressText = CT(goPanel.transform, "GS", "", 40, jpFont, new Vector2(0.5f,0.5f), new Vector2(0.5f,0.5f), new Vector2(0.5f,0.5f), new Vector2(400,60), Vector2.zero);
        goProgressText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        ...
        uiSO.FindProperty("_gameOverProgressText").objectReferenceValue = goProgressText.GetComponent<TextMeshProUGUI>();
        ...
        gmSO.FindProperty("_sweepManager").objectReferenceValue = sm;
        ...
        string scenePath = "Assets/Scenes/055_DustSweep.unity";
        EditorSceneManager.SaveScene(scene, scenePath);
        AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup055] DustSweep シーンを作成しました: " + scenePath);
    }
