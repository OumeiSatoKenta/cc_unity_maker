using UnityEngine;using UnityEngine.UI;using UnityEditor;using UnityEditor.SceneManagement;using UnityEngine.InputSystem.UI;using TMPro;using Game036_CoinStack;
public static class Setup036_CoinStack
{
    [MenuItem("Assets/Setup/036 CoinStack")]
    public static void CreateScene()
    {
        if(EditorApplication.isPlaying){Debug.LogError("[Setup036_CoinStack] Play モード中は実行できません。");return;}
        ...
        var camera=Object.FindFirstObjectByType<Camera>();if(camera!=null){camera.backgroundColor=new Color(0.08f,0.08f,0.12f,1f);camera.orthographic=true;camera.orthographicSize=5f;}
        string whiteTexPath="Assets/Scripts/Game036_CoinStack/WhiteSquare.png";
        ...
        var whiteSprite=AssetDatabase.LoadAssetAtPath<Sprite>(whiteTexPath);string sp="Assets/Resources/Sprites/Game036_CoinStack/";string pd="Assets/Scripts/Game036_CoinStack/";
        var coinPrefab=SP(pd+"CoinPrefab.prefab",AssetDatabase.LoadAssetAtPath<Sprite>(sp+"coin.png"),whiteSprite,5);
        // Platform at bottom
        var plat=new GameObject("Platform");var psr=plat.AddComponent<SpriteRenderer>();psr.sprite=whiteSprite;psr.color=new Color(0.3f,0.32f,0.4f);psr.sortingOrder=-3;plat.transform.position=new Vector3(0,-3.7f,0);plat.transform.localScale=new Vector3(4f,0.3f,1);

        var gmObj=new GameObject("GameManager");var gm=gmObj.AddComponent<CoinStackGameManager>();
        ...
        var smSO=new SerializedObject(sm);smSO.FindProperty("_coinPrefab").objectReferenceValue=coinPrefab;smSO.FindProperty("_dropSpeed").floatValue=4f;
        smSO.FindProperty("_swingSpeed").floatValue=3f;smSO.FindProperty("_coinHeight").floatValue=0.25f;smSO.ApplyModifiedProperties();
        ...
        var heightText=CT(canvasObj.transform,"HeightText","高さ: 0",32,jpFont,new Vector2(0,1),new Vector2(0,1),new Vector2(0,1),new Vector2(200,50),new Vector2(20,-20));
        var perfectText=CT(canvasObj.transform,"PerfectText","Perfect: 0",28,jpFont,new Vector2(0,1),new Vector2(0,1),new Vector2(0,1),new Vector2(200,40),new Vector2(20,-65));
        perfectText.GetComponent<TextMeshProUGUI>().color=new Color(1f,0.85f,0.3f);
        var hint=CT(canvasObj.transform,"HintText","タップでコインを落とす!",20,jpFont,new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(400,40),new Vector2(0,20));
        ...
        var goText=CT(goPanel.transform,"GameOverText","崩壊!",48,jpFont,new Vector2(0.5f,0.6f),new Vector2(0.5f,0.6f),new Vector2(0.5f,0.5f),new Vector2(400,200),Vector2.zero);
        goText.GetComponent<TextMeshProUGUI>().alignment=TextAlignmentOptions.Center;
        var retryBtn=CB(goPanel.transform,"RetryButton","リトライ",28,jpFont,new Vector2(0.5f,0.15f),new Vector2(0.5f,0.15f),new Vector2(0.5f,0.5f),new Vector2(200,60),Vector2.zero,new Color(0.7f,0.6f,0.2f,1f));
        goPanel.SetActive(false);

        var uiObj=new GameObject("CoinStackUI");var csUI=uiObj.AddComponent<CoinStackUI>();
        var uiSO=new SerializedObject(csUI);uiSO.FindProperty("_heightText").objectReferenceValue=heightText.GetComponent<TextMeshProUGUI>();
        ...
        menuBtn.AddComponent<BackToMenuButton>();var gmSO=new SerializedObject(gm);gmSO.FindProperty("_stackManager").objectReferenceValue=sm;gmSO.FindProperty("_ui").objectReferenceValue=csUI;gmSO.ApplyModifiedProperties();
        ...
        string scenePath="Assets/Scenes/036_CoinStack.unity";EditorSceneManager.SaveScene(scene,scenePath);AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup036_CoinStack] CoinStack シーンを作成しました: "+scenePath);
    }
