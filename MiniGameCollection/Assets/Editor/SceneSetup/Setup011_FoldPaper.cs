...
public static class Setup011_FoldPaper
{
    ...
    public static void CreateScene()
    {
        ...
        string sp = "Assets/Resources/Sprites/Game011_FoldPaper/";
        var paperSprite = AssetDatabase.LoadAssetAtPath<Sprite>(sp + "paper_white.png");
        var targetSprite = AssetDatabase.LoadAssetAtPath<Sprite>(sp + "target_mark.png");

        string pd = "Assets/Scripts/Game011_FoldPaper/";

        // Cell prefab
        var cellObj = new GameObject("CellPrefab");
        var csr = cellObj.AddComponent<SpriteRenderer>();
        csr.sprite = paperSprite != null ? paperSprite : whiteSprite;
        csr.sortingOrder = 5;
        var col = cellObj.AddComponent<BoxCollider2D>();
        col.size = new Vector2(0.9f, 0.9f);
        cellObj.AddComponent<PaperCell>();
        var cellPrefab = PrefabUtility.SaveAsPrefabAsset(cellObj, pd + "CellPrefab.prefab");
        Object.DestroyImmediate(cellObj);

        // Target mark prefab
        var tmObj = new GameObject("TargetMarkPrefab");
        var tmsr = tmObj.AddComponent<SpriteRenderer>();
        tmsr.sprite = targetSprite != null ? targetSprite : whiteSprite;
        tmsr.sortingOrder = 8;
        var tmPrefab = PrefabUtility.SaveAsPrefabAsset(tmObj, pd + "TargetMarkPrefab.prefab");
        Object.DestroyImmediate(tmObj);

        // GameManager + FoldManager
        var gmObj = new GameObject("GameManager");
        var gm = gmObj.AddComponent<FoldPaperGameManager>();
        var boardObj = new GameObject("FoldBoard");
        boardObj.transform.SetParent(gmObj.transform);
        var fm = boardObj.AddComponent<FoldManager>();
        ...
        fmSO.FindProperty("_targetMarkPrefab").objectReferenceValue = tmPrefab;
        ...
        var hintText = CreateText(canvasObj.transform, "HintText", "セルをタップで十字型に折る。右の目標パターンを作ろう", 20, jpFont,
        ...
        hintText.GetComponent<TextMeshProUGUI>().color = new Color(0.7f,0.65f,0.55f);
        ...
        gmSO.FindProperty("_foldManager").objectReferenceValue = fm;
