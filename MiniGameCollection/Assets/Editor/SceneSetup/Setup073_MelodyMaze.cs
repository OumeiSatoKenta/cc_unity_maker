...
using TMPro;
using System.IO;
using Game073_MelodyMaze;

public static class Setup073_MelodyMaze
{
    [MenuItem("Assets/Setup/073 MelodyMaze")]
    public static void CreateScene()
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup073] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game073_MelodyMaze/";

        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.1f, 0.08f, 0.16f); camera.orthographic = true; camera.orthographicSize = 5.5f; }

        Sprite bgSprite = LoadSprite(sp + "background.png");
        Sprite wallSprite = LoadSprite(sp + "wall.png");
        Sprite pathSprite = LoadSprite(sp + "path.png");
        ...
        var mmObj = new GameObject("MazeManager"); mmObj.transform.SetParent(gmObj.transform);
        ...
        mmSO.FindProperty("_pathSprite").objectReferenceValue = pathSprite;
        mmSO.FindProperty("_noteSprite").objectReferenceValue = noteSprite;
        mmSO.FindProperty("_playerSprite").objectReferenceValue = playerSprite;
        ...
        var noteText = CT(canvasObj.transform, "NoteText", "♪ 0/5", 36, jpFont, new Vector2(0,1), new Vector2(0,1), new Vector2(0,1), new Vector2(200,50), new Vector2(20,-15));
        noteText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Left;
        noteText.GetComponent<TextMeshProUGUI>().color = new Color(1f, 0.85f, 0.2f);

        var moveText = CT(canvasObj.transform, "MoveText", "0歩", 32, jpFont, new Vector2(1,1), new Vector2(1,1), new Vector2(1,1), new Vector2(150,40), new Vector2(-20,-15));
        moveText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Right;
        ...
        var goPanel = CreatePanel(canvasObj.transform, "GameOverPanel", new Color(0.15f,0.08f,0.08f,0.95f));
        CT(goPanel.transform, "GT", "音符が足りない…", 44, jpFont, new Vector2(0.5f,0.7f), new Vector2(0.5f,0.7f), new Vector2(0.5f,0.5f), new Vector2(600,80), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        ...
        var goRetryBtn = CB(goPanel.transform, "RetryButton", "リトライ", 32, jpFont, new Vector2(0.5f,0.25f), new Vector2(0.5f,0.25f), new Vector2(0.5f,0.5f), new Vector2(220,65), Vector2.zero, new Color(0.5f,0.2f,0.2f));
        goPanel.SetActive(false);

        var uiObj = new GameObject("MelodyMazeUI"); uiObj.transform.SetParent(gmObj.transform);
        var ui = uiObj.AddComponent<MelodyMazeUI>();
        var uiSO = new SerializedObject(ui);
        uiSO.FindProperty("_noteText").objectReferenceValue = noteText.GetComponent<TextMeshProUGUI>();
        ...
        gmSO.FindProperty("_targetNotes").intValue = 5;
        ...
        string scenePath = "Assets/Scenes/073_MelodyMaze.unity";
        EditorSceneManager.SaveScene(scene, scenePath);
        AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup073] MelodyMaze シーンを作成しました: " + scenePath);
    }
