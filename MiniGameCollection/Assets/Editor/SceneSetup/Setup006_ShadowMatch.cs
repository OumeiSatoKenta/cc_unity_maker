...
public static class Setup006_ShadowMatch
{
    ...
    public static void CreateScene()
    {
        ...
        {
            Debug.LogError("[Setup006_ShadowMatch] Play モード中は実行できません。");
            return;
        }
        ...
        if (camera != null)
        {
            camera.backgroundColor = new Color(0.07f, 0.06f, 0.12f, 1f);
            camera.orthographic = true;
            camera.orthographicSize = 5f;
        }

        // White sprite
        string whiteTexPath = "Assets/Scripts/Game006_ShadowMatch/WhiteSquare.png";
        ...
        string sp = "Assets/Resources/Sprites/Game006_ShadowMatch/";
        var shapeTriSprite = AssetDatabase.LoadAssetAtPath<Sprite>(sp + "shape_triangle.png");
        var shadowTriSprite = AssetDatabase.LoadAssetAtPath<Sprite>(sp + "shadow_triangle.png");
        var targetFrameSprite = AssetDatabase.LoadAssetAtPath<Sprite>(sp + "target_frame.png");

        // Target shadow (left side)
        var targetObj = new GameObject("TargetShadow");
        var targetSr = targetObj.AddComponent<SpriteRenderer>();
        targetSr.sprite = shadowTriSprite != null ? shadowTriSprite : whiteSprite;
        targetSr.color = Color.white;
        targetSr.sortingOrder = 2;
        targetObj.transform.position = new Vector3(-3f, 0f, 0f);
        targetObj.transform.localScale = Vector3.one * 2f;

        // Target frame
        var frameObj = new GameObject("TargetFrame");
        var frameSr = frameObj.AddComponent<SpriteRenderer>();
        frameSr.sprite = targetFrameSprite != null ? targetFrameSprite : whiteSprite;
        frameSr.color = targetFrameSprite != null ? Color.white : new Color(1f, 0.8f, 0.2f, 0.5f);
        frameSr.sortingOrder = 3;
        frameObj.transform.position = new Vector3(-3f, 0f, 0f);
        frameObj.transform.localScale = Vector3.one * 2.2f;

        // Shape (right side, rotatable)
        var shapeObj = new GameObject("Shape");
        var shapeSr = shapeObj.AddComponent<SpriteRenderer>();
        shapeSr.sprite = shapeTriSprite != null ? shapeTriSprite : whiteSprite;
        shapeSr.color = Color.white;
        shapeSr.sortingOrder = 5;
        shapeObj.transform.position = new Vector3(3f, 0f, 0f);
        shapeObj.transform.localScale = Vector3.one * 2f;
        var col = shapeObj.AddComponent<CircleCollider2D>();
        col.radius = 1f;
        var shapeCtrl = shapeObj.AddComponent<ShapeController>();

        // Labels
        var labelTarget = new GameObject("LabelTarget");
        var ltSr = labelTarget.AddComponent<SpriteRenderer>();
        ltSr.sprite = whiteSprite;
        ltSr.color = new Color(0, 0, 0, 0); // invisible, just for organization

        // GameManager + ShadowManager
        var gmObj = new GameObject("GameManager");
        var gm = gmObj.AddComponent<ShadowMatchGameManager>();
        var boardObj = new GameObject("ShadowBoard");
        boardObj.transform.SetParent(gmObj.transform);
        var sm = boardObj.AddComponent<ShadowManager>();

        var smSO = new SerializedObject(sm);
        smSO.FindProperty("_shape").objectReferenceValue = shapeCtrl;
        smSO.FindProperty("_targetShadow").objectReferenceValue = targetSr;
        smSO.FindProperty("_matchThreshold").floatValue = 15f;
        ...
        var moveText = CreateText(canvasObj.transform, "MoveCountText", "操作: 0", 32, jpFont,
        ...
        var hintText = CreateText(canvasObj.transform, "HintText", "右の図形をドラッグで回転させて影を合わせよう", 20, jpFont,
        ...
        gmSO.FindProperty("_shadowManager").objectReferenceValue = sm;
        ...
        Object.DestroyImmediate(labelTarget);

        string scenePath = "Assets/Scenes/006_ShadowMatch.unity";
        EditorSceneManager.SaveScene(scene, scenePath);
        AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup006_ShadowMatch] ShadowMatch シーンを作成しました: " + scenePath);
    }
