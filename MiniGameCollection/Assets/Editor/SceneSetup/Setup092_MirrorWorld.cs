...
using TMPro;
using System.IO;
using Game092_MirrorWorld;

public static class Setup092_MirrorWorld
{
    [MenuItem("Assets/Setup/092 MirrorWorld")]
    public static void CreateScene()
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup092] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game092_MirrorWorld/";

        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.12f, 0.13f, 0.18f); camera.orthographic = true; camera.orthographicSize = 6f; }

        Sprite bgSprite = LoadSprite(sp + "background.png");
        Sprite playerTopSprite = LoadSprite(sp + "player_top.png");
        Sprite playerBotSprite = LoadSprite(sp + "player_bot.png");
        ...
        var gmObj = new GameObject("GameManager"); var gm = gmObj.AddComponent<MirrorWorldGameManager>();
        var mmObj = new GameObject("MirrorManager"); mmObj.transform.SetParent(gmObj.transform);
        var mm = mmObj.AddComponent<MirrorManager>();
        var mmSO = new SerializedObject(mm);
        mmSO.FindProperty("_gameManager").objectReferenceValue = gm;
        mmSO.FindProperty("_playerTopSprite").objectReferenceValue = playerTopSprite;
        mmSO.FindProperty("_playerBotSprite").objectReferenceValue = playerBotSprite;
        mmSO.FindProperty("_goalSprite").objectReferenceValue = goalSprite;
        mmSO.FindProperty("_trapSprite").objectReferenceValue = trapSprite;
        mmSO.FindProperty("_wallSprite").objectReferenceValue = wallSprite;
        ...
        var stageText = CT(canvasObj.transform, "StageText", "Stage 1/5", 32, jpFont, new Vector2(0,1), new Vector2(0,1), new Vector2(0,1), new Vector2(220,40), new Vector2(20,-15));
        stageText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Left; stageText.GetComponent<TextMeshProUGUI>().color = Color.white;
        var moveText = CT(canvasObj.transform, "MoveText", "0手", 28, jpFont, new Vector2(1,1), new Vector2(1,1), new Vector2(1,1), new Vector2(120,35), new Vector2(-20,-15));
        ...
        CT(clearPanel.transform, "CT", "ミラークリア！", 48, jpFont, new Vector2(0.5f,0.75f), new Vector2(0.5f,0.75f), new Vector2(0.5f,0.5f), new Vector2(600,80), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        ...
        CT(goPanel.transform, "GT", "トラップ！", 48, jpFont, new Vector2(0.5f,0.7f), new Vector2(0.5f,0.7f), new Vector2(0.5f,0.5f), new Vector2(600,80), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        ...
        var uiObj = new GameObject("MirrorWorldUI"); uiObj.transform.SetParent(gmObj.transform);
        ...
        gmSO.FindProperty("_mirrorManager").objectReferenceValue = mm;
        gmSO.FindProperty("_ui").objectReferenceValue = ui;
        gmSO.FindProperty("_totalStages").intValue = 5;
        ...
        string scenePath = "Assets/Scenes/092_MirrorWorld.unity";
        EditorSceneManager.SaveScene(scene, scenePath); AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup092] MirrorWorld シーンを作成しました: " + scenePath);
    }
