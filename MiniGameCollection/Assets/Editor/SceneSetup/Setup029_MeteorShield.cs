...
public static class Setup029_MeteorShield
{
    ...
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup029_MeteorShield] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.02f, 0.02f, 0.08f, 1f); camera.orthographic = true; camera.orthographicSize = 5f; }
        ...
        var meteorPrefab = SP(pd + "MeteorPrefab.prefab", AssetDatabase.LoadAssetAtPath<Sprite>(sp + "meteor.png"), whiteSprite, 5);
        var shieldPrefab = SP(pd + "ShieldPrefab.prefab", AssetDatabase.LoadAssetAtPath<Sprite>(sp + "shield.png"), whiteSprite, 10);

        // Star at bottom center
        var starSprite = AssetDatabase.LoadAssetAtPath<Sprite>(sp + "star_base.png");
        var starObj = new GameObject("StarBase");
        var ssr = starObj.AddComponent<SpriteRenderer>(); ssr.sprite = starSprite ?? whiteSprite; ssr.sortingOrder = 3;
        starObj.transform.position = new Vector3(0, -4f, 0);

        var gmObj = new GameObject("GameManager"); var gm = gmObj.AddComponent<MeteorShieldGameManager>();
        var boardObj = new GameObject("ShieldBoard"); boardObj.transform.SetParent(gmObj.transform);
        var sm = boardObj.AddComponent<ShieldManager>();
        var smSO = new SerializedObject(sm);
        smSO.FindProperty("_meteorPrefab").objectReferenceValue = meteorPrefab;
        smSO.FindProperty("_shieldPrefab").objectReferenceValue = shieldPrefab;
        smSO.FindProperty("_spawnInterval").floatValue = 0.8f;
        smSO.FindProperty("_meteorSpeed").floatValue = 4f;
        ...
        var hpText = CT(canvasObj.transform, "HPText", "星HP: 5", 28, jpFont, new Vector2(0,1), new Vector2(0,1), new Vector2(0,1), new Vector2(200,40), new Vector2(20,-65));
        hpText.GetComponent<TextMeshProUGUI>().color = new Color(1f, 0.85f, 0.3f);
        ...
        var retryBtn = CB(goPanel.transform, "RetryButton", "リトライ", 28, jpFont, new Vector2(0.5f,0.2f), new Vector2(0.5f,0.2f), new Vector2(0.5f,0.5f), new Vector2(200,60), Vector2.zero, new Color(0.4f,0.2f,0.6f,1f));
        goPanel.SetActive(false);

        var uiObj = new GameObject("MeteorShieldUI"); var msUI = uiObj.AddComponent<MeteorShieldUI>();
        ...
        Debug.Log("[Setup029_MeteorShield] MeteorShield シーンを作成しました: " + scenePath);
    }
