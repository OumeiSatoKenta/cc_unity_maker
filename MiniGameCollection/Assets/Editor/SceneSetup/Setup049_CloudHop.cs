using UnityEngine;using UnityEngine.UI;using UnityEditor;using UnityEditor.SceneManagement;using UnityEngine.InputSystem.UI;using TMPro;using Game049_CloudHop;
public static class Setup049_CloudHop
{
    ...
    public static void CreateScene()
    {
        ...
        var camera=Object.FindFirstObjectByType<Camera>();if(camera!=null){camera.backgroundColor=new Color(0.5f,0.7f,0.95f,1f);camera.orthographic=true;camera.orthographicSize=5f;}

        var gmObj=new GameObject("GameManager");var gm=gmObj.AddComponent<CloudHopGameManager>();
        var boardObj=new GameObject("CloudBoard");boardObj.transform.SetParent(gmObj.transform);var cm=boardObj.AddComponent<CloudManager>();
        ...
        var scoreText=CT(canvasObj.transform,"ScoreText","高さ: 0",36,jpFont,new Vector2(0.5f,1),new Vector2(0.5f,1),new Vector2(0.5f,1),new Vector2(250,50),new Vector2(0,-20));
        scoreText.GetComponent<TextMeshProUGUI>().alignment=TextAlignmentOptions.Center;scoreText.GetComponent<TextMeshProUGUI>().color=new Color(0.2f,0.2f,0.4f);
        var hint=CT(canvasObj.transform,"HintText","左右移動+スペース/タップでジャンプ",18,jpFont,new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(500,35),new Vector2(0,20));
        ...
        var goPanel=new GameObject("GameOverPanel",typeof(RectTransform));goPanel.transform.SetParent(canvasObj.transform,false);goPanel.AddComponent<Image>().color=new Color(0,0,0,0.8f);
        ...
        var finalText=CT(goPanel.transform,"FinalScore","高さ: 0",36,jpFont,new Vector2(0.5f,0.45f),new Vector2(0.5f,0.45f),new Vector2(0.5f,0.5f),new Vector2(300,50),Vector2.zero);
        finalText.GetComponent<TextMeshProUGUI>().alignment=TextAlignmentOptions.Center;finalText.GetComponent<TextMeshProUGUI>().color=new Color(1f,0.9f,0.3f);
        ...
        var uiObj=new GameObject("CloudHopUI");var chUI=uiObj.AddComponent<CloudHopUI>();
        var uiSO=new SerializedObject(chUI);uiSO.FindProperty("_scoreText").objectReferenceValue=scoreText.GetComponent<TextMeshProUGUI>();
        ...
        menuBtn.AddComponent<BackToMenuButton>();var gmSO=new SerializedObject(gm);gmSO.FindProperty("_cloudManager").objectReferenceValue=cm;gmSO.FindProperty("_ui").objectReferenceValue=chUI;gmSO.ApplyModifiedProperties();

        retryBtn.GetComponent<Button>().onClick.AddListener(chUI.OnRetryButton);
        ...
        string scenePath="Assets/Scenes/049_CloudHop.unity";EditorSceneManager.SaveScene(scene,scenePath);AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup049_CloudHop] シーンを作成しました: "+scenePath);
    }
