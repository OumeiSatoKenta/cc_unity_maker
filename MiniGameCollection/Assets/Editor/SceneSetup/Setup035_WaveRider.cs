using UnityEngine;using UnityEngine.UI;using UnityEditor;using UnityEditor.SceneManagement;using UnityEngine.InputSystem.UI;using TMPro;using Game035_WaveRider;
public static class Setup035_WaveRider
{
    [MenuItem("Assets/Setup/035 WaveRider")]
    public static void CreateScene()
    {
        if (EditorApplication.isPlaying){Debug.LogError("[Setup035_WaveRider] Play モード中は実行できません。");return;}
        ...
        var camera=Object.FindFirstObjectByType<Camera>();if(camera!=null){camera.backgroundColor=new Color(0.15f,0.3f,0.6f,1f);camera.orthographic=true;camera.orthographicSize=5f;}
        string whiteTexPath="Assets/Scripts/Game035_WaveRider/WhiteSquare.png";
        ...
        var whiteSprite=AssetDatabase.LoadAssetAtPath<Sprite>(whiteTexPath);string sp="Assets/Resources/Sprites/Game035_WaveRider/";string pd="Assets/Scripts/Game035_WaveRider/";
        var surferPrefab=SP(pd+"SurferPrefab.prefab",AssetDatabase.LoadAssetAtPath<Sprite>(sp+"surfer.png"),whiteSprite,10);
        var rockPrefab=SP(pd+"RockPrefab.prefab",AssetDatabase.LoadAssetAtPath<Sprite>(sp+"rock.png"),whiteSprite,5);
        // Water line
        var water=new GameObject("Water");var wsr=water.AddComponent<SpriteRenderer>();wsr.sprite=whiteSprite;wsr.color=new Color(0.2f,0.4f,0.7f,0.5f);wsr.sortingOrder=-5;water.transform.position=new Vector3(0,-4.5f,0);water.transform.localScale=new Vector3(15f,2f,1);

        var gmObj=new GameObject("GameManager");var gm=gmObj.AddComponent<WaveRiderGameManager>();
        var boardObj=new GameObject("WaveBoard");boardObj.transform.SetParent(gmObj.transform);var wm=boardObj.AddComponent<WaveManager>();
        var wmSO=new SerializedObject(wm);wmSO.FindProperty("_surferPrefab").objectReferenceValue=surferPrefab;wmSO.FindProperty("_rockPrefab").objectReferenceValue=rockPrefab;
        wmSO.FindProperty("_scrollSpeed").floatValue=4f;wmSO.FindProperty("_spawnInterval").floatValue=1.5f;wmSO.ApplyModifiedProperties();
        ...
        var scoreText=CT(canvasObj.transform,"ScoreText","トリック: 0",28,jpFont,new Vector2(0,1),new Vector2(0,1),new Vector2(0,1),new Vector2(220,40),new Vector2(20,-20));
        var distText=CT(canvasObj.transform,"DistanceText","距離: 0m",32,jpFont,new Vector2(0.5f,1),new Vector2(0.5f,1),new Vector2(0.5f,1),new Vector2(250,50),new Vector2(0,-20));
        distText.GetComponent<TextMeshProUGUI>().alignment=TextAlignmentOptions.Center;
        var hint=CT(canvasObj.transform,"HintText","長押しで上昇、離すと下降。岩を避けろ!",20,jpFont,new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(600,40),new Vector2(0,20));
        hint.GetComponent<TextMeshProUGUI>().alignment=TextAlignmentOptions.Center;hint.GetComponent<TextMeshProUGUI>().color=new Color(0.5f,0.7f,0.9f);
        ...
        var goText=CT(goPanel.transform,"GameOverText","クラッシュ!",48,jpFont,new Vector2(0.5f,0.6f),new Vector2(0.5f,0.6f),new Vector2(0.5f,0.5f),new Vector2(400,200),Vector2.zero);
        goText.GetComponent<TextMeshProUGUI>().alignment=TextAlignmentOptions.Center;
        var retryBtn=CB(goPanel.transform,"RetryButton","リトライ",28,jpFont,new Vector2(0.5f,0.15f),new Vector2(0.5f,0.15f),new Vector2(0.5f,0.5f),new Vector2(200,60),Vector2.zero,new Color(0.2f,0.4f,0.7f,1f));
        goPanel.SetActive(false);

        var uiObj=new GameObject("WaveRiderUI");var wrUI=uiObj.AddComponent<WaveRiderUI>();
        var uiSO=new SerializedObject(wrUI);uiSO.FindProperty("_scoreText").objectReferenceValue=scoreText.GetComponent<TextMeshProUGUI>();
        ...
        menuBtn.AddComponent<BackToMenuButton>();var gmSO=new SerializedObject(gm);gmSO.FindProperty("_waveManager").objectReferenceValue=wm;gmSO.FindProperty("_ui").objectReferenceValue=wrUI;gmSO.ApplyModifiedProperties();
        ...
        string scenePath="Assets/Scenes/035_WaveRider.unity";EditorSceneManager.SaveScene(scene,scenePath);AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup035_WaveRider] WaveRider シーンを作成しました: "+scenePath);
    }
