...
using TMPro;
using System.IO;
using Game014v2_MagnetPath;

public static class Setup014v2_MagnetPath
{
    [MenuItem("Assets/Setup/014v2 MagnetPath")]
    public static void CreateScene()
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup014v2] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game014v2_MagnetPath/";
        ...
        Sprite spMagnetN     = LoadSprite(sp + "MagnetN.png");
        Sprite spMagnetS     = LoadSprite(sp + "MagnetS.png");
        Sprite spMagnetNLg   = LoadSprite(sp + "MagnetNLarge.png");
        Sprite spMagnetSLg   = LoadSprite(sp + "MagnetSLarge.png");
        Sprite spMagnetSw    = LoadSprite(sp + "MagnetSwitch.png");
        Sprite spBall        = LoadSprite(sp + "Ball.png");
        Sprite spBall2       = LoadSprite(sp + "Ball2.png");
        Sprite spGoal        = LoadSprite(sp + "Goal.png");
        Sprite spGoal2       = LoadSprite(sp + "Goal2.png");
        ...
        if (spBg != null)
        {
            ...
            bgObj.transform.localScale = new Vector3(0.06f, 0.06f, 1f);
        }
        ...
        // MagnetManager (child)
        var magObj = new GameObject("MagnetManager");
        magObj.transform.SetParent(gmObj.transform);
        var magMgr = magObj.AddComponent<MagnetManager>();
        ...
        // Switch count (top center)
        var switchText = CT(canvasObj.transform, "SwitchCountText", "切替: 5/5", 34, jpFont,
            new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f),
            new Vector2(400, 55), new Vector2(0, -30));
        switchText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        switchText.GetComponent<TextMeshProUGUI>().color = new Color(1f, 0.75f, 0.3f);

        // Combo text (top center, below switch)
        var comboText = CT(canvasObj.transform, "ComboText", "", 30, jpFont,
            new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f),
            new Vector2(400, 45), new Vector2(0, -85));
        ...
        var startBtn = CB(canvasObj.transform, "StartButton", "発射！", jpFont,
        ...
        var gcTotalScore = CT(gcPanel.transform, "ClearTotalScoreText", "Total: 0", 44, jpFont,
            new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
            new Vector2(500, 60), new Vector2(0, 20));
        gcTotalScore.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        gcTotalScore.GetComponent<TextMeshProUGUI>().color = new Color(1f, 0.9f, 0.3f);
        ...
        var ipTitle = CT(ipPanel.transform, "IPTitle", "MagnetPath", 60, jpFont,
            new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
        ...
        uiSO.FindProperty("_switchCountText").objectReferenceValue = switchText.GetComponent<TextMeshProUGUI>();
        ...
        uiSO.FindProperty("_clearTotalScoreText").objectReferenceValue = gcTotalScore.GetComponent<TextMeshProUGUI>();
        uiSO.FindProperty("_gameManager").objectReferenceValue = gm;
        uiSO.FindProperty("_magnetManager").objectReferenceValue = magMgr;
        uiSO.ApplyModifiedProperties();

        // ---- Wire MagnetManager ----
        var magSO = new SerializedObject(magMgr);
        magSO.FindProperty("_gameManager").objectReferenceValue = gm;
        magSO.FindProperty("_magnetNSprite").objectReferenceValue = spMagnetN;
        magSO.FindProperty("_magnetSSprite").objectReferenceValue = spMagnetS;
        magSO.FindProperty("_magnetNLargeSprite").objectReferenceValue = spMagnetNLg;
        magSO.FindProperty("_magnetSLargeSprite").objectReferenceValue = spMagnetSLg;
        magSO.FindProperty("_magnetSwitchSprite").objectReferenceValue = spMagnetSw;
        magSO.FindProperty("_ballSprite").objectReferenceValue = spBall;
        magSO.FindProperty("_ball2Sprite").objectReferenceValue = spBall2;
        magSO.FindProperty("_goalSprite").objectReferenceValue = spGoal;
        magSO.FindProperty("_goal2Sprite").objectReferenceValue = spGoal2;
        magSO.FindProperty("_wallSprite").objectReferenceValue = spWall;
        magSO.ApplyModifiedProperties();

        // ---- Wire GameManager ----
        var gmSO = new SerializedObject(gm);
        gmSO.FindProperty("_stageManager").objectReferenceValue = sm;
        gmSO.FindProperty("_instructionPanel").objectReferenceValue = ip;
        gmSO.FindProperty("_magnetManager").objectReferenceValue = magMgr;
        gmSO.FindProperty("_ui").objectReferenceValue = mpUI;
        gmSO.ApplyModifiedProperties();

        // ---- Button Events ----
        AddButtonOnClick(startBtn.GetComponent<Button>(), mpUI, "OnStartButton");
        AddButtonOnClick(resetBtn.GetComponent<Button>(), mpUI, "OnResetButton");
        ...
        // ---- Save Scene ----
        string scenePath = "Assets/Scenes/014v2_MagnetPath.unity";
        EditorSceneManager.SaveScene(scene, scenePath);
        AddSceneToBuildSettings(scenePath);
        AssetDatabase.Refresh();
        Debug.Log("[Setup014v2] Scene created: " + scenePath);
    }
    ...
    static Sprite LoadSprite(string path)
    {
        ...
        if (tex == null)
        {
            Debug.LogWarning($"[Setup014v2] Sprite not found: {path}");
            return null;
        }
