...
using TMPro;
using System.IO;
using Game060_MeltIce;

public static class Setup060_MeltIce
{
    [MenuItem("Assets/Setup/060 MeltIce")]
    public static void CreateScene()
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup060] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game060_MeltIce/";

        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.2f, 0.22f, 0.35f); camera.orthographic = true; camera.orthographicSize = 5.5f; }

        Sprite bgSprite = LoadSprite(sp + "background.png");
        Sprite iceSprite = LoadSprite(sp + "ice.png");
        ...
        mmSO.FindProperty("_iceSprite").objectReferenceValue = iceSprite;
        mmSO.FindProperty("_mirrorSprite").objectReferenceValue = mirrorSprite;
        mmSO.FindProperty("_sunSprite").objectReferenceValue = sunSprite;
        ...
        var mirrorsText = CT(canvasObj.transform, "MirrorsText", "ミラー: 0/5", 36, jpFont, new Vector2(1,1), new Vector2(1,1), new Vector2(1,1), new Vector2(200,50), new Vector2(-20,-20));
        mirrorsText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Right;
        mirrorsText.GetComponent<TextMeshProUGUI>().color = Color.white;

        var iceText = CT(canvasObj.transform, "IceText", "", 36, jpFont, new Vector2(0,1), new Vector2(0,1), new Vector2(0,1), new Vector2(200,50), new Vector2(20,-20));
        iceText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Left;
        iceText.GetComponent<TextMeshProUGUI>().color = Color.white;
        ...
        var clearPanel = CreatePanel(canvasObj.transform, "ClearPanel", new Color(0.85f,0.9f,1f,0.95f));
        ...
        var uiObj = new GameObject("MeltIceUI"); uiObj.transform.SetParent(gmObj.transform);
        var ui = uiObj.AddComponent<MeltIceUI>();
        var uiSO = new SerializedObject(ui);
        uiSO.FindProperty("_mirrorsText").objectReferenceValue = mirrorsText.GetComponent<TextMeshProUGUI>();
        uiSO.FindProperty("_iceText").objectReferenceValue = iceText.GetComponent<TextMeshProUGUI>();
        ...
        gmSO.FindProperty("_maxMirrors").intValue = 5;
        ...
        string scenePath = "Assets/Scenes/060_MeltIce.unity";
        EditorSceneManager.SaveScene(scene, scenePath);
        AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup060] MeltIce シーンを作成しました: " + scenePath);
    }
