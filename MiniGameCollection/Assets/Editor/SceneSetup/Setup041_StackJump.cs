...
public static class Setup041_StackJump
{
    ...
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup041] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game041_StackJump/";

        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.08f, 0.08f, 0.2f); camera.orthographic = true; camera.orthographicSize = 5.5f; }
        ...
        var smObj = new GameObject("StackJumpManager"); smObj.transform.SetParent(gmObj.transform);
        var sm = smObj.AddComponent<StackJumpManager>();
        var smSO = new SerializedObject(sm);
        smSO.FindProperty("_gameManager").objectReferenceValue = gm;
        smSO.FindProperty("_blockSprite").objectReferenceValue = blockSprite;
        ...
        var heightText = CT(canvasObj.transform, "HeightText", "0/20", 60, jpFont, new Vector2(0.5f,1), new Vector2(0.5f,1), new Vector2(0.5f,1), new Vector2(200,70), new Vector2(0,-30));
        ...
        var clearPanel = CreatePanel(canvasObj.transform, "ClearPanel", new Color(0.1f,0.3f,0.2f,0.9f));
        ...
        var clearRetryBtn = CB(clearPanel.transform, "RetryButton", "もう一度", 32, jpFont, new Vector2(0.5f,0.25f), new Vector2(0.5f,0.25f), new Vector2(0.5f,0.5f), new Vector2(220,65), Vector2.zero, new Color(0.1f,0.4f,0.2f));
        clearPanel.SetActive(false);

        var goPanel = CreatePanel(canvasObj.transform, "GameOverPanel", new Color(0.3f,0.05f,0f,0.9f));
        CT(goPanel.transform, "GT", "崩壊！", 52, jpFont, new Vector2(0.5f,0.75f), new Vector2(0.5f,0.75f), new Vector2(0.5f,0.5f), new Vector2(600,100), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        var goScoreText = CT(goPanel.transform, "GS", "", 40, jpFont, new Vector2(0.5f,0.55f), new Vector2(0.5f,0.55f), new Vector2(0.5f,0.5f), new Vector2(400,60), Vector2.zero);
        ...
        var uiObj = new GameObject("StackJumpUI"); uiObj.transform.SetParent(gmObj.transform);
        var ui = uiObj.AddComponent<StackJumpUI>();
        ...
        gmSO.FindProperty("_goalHeight").intValue = 20;
        ...
        string scenePath = "Assets/Scenes/041_StackJump.unity";
        EditorSceneManager.SaveScene(scene, scenePath); AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup041] StackJump シーンを作成しました: " + scenePath);
    }
