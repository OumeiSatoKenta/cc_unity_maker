...
public static class Setup038_FlyBird
{
    ...
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup038] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game038_FlyBird/";

        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.53f, 0.78f, 0.92f); camera.orthographic = true; camera.orthographicSize = 5.5f; }

        Sprite bgSprite = LoadSprite(sp + "background.png");
        Sprite birdSprite = LoadSprite(sp + "bird.png");
        Sprite pipeSprite = LoadSprite(sp + "pipe.png");
        ...
        birdObj.transform.position = new Vector3(-2f, 0f, 0f);
        birdObj.transform.localScale = new Vector3(0.8f, 0.8f, 1f);
        var birdSr = birdObj.AddComponent<SpriteRenderer>(); birdSr.sprite = birdSprite; birdSr.sortingOrder = 5;

        var gmObj = new GameObject("GameManager");
        var gm = gmObj.AddComponent<FlyBirdGameManager>();

        var fmObj = new GameObject("FlyManager"); fmObj.transform.SetParent(gmObj.transform);
        var fm = fmObj.AddComponent<FlyManager>();
        var fmSO = new SerializedObject(fm);
        fmSO.FindProperty("_gameManager").objectReferenceValue = gm;
        fmSO.FindProperty("_birdTransform").objectReferenceValue = birdObj.transform;
        fmSO.FindProperty("_pipeSprite").objectReferenceValue = pipeSprite;
        ...
        var scoreText = CT(canvasObj.transform, "ScoreText", "0", 72, jpFont, new Vector2(0.5f,1), new Vector2(0.5f,1), new Vector2(0.5f,1), new Vector2(200,80), new Vector2(0,-40));
        scoreText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        var distText = CT(canvasObj.transform, "DistanceText", "0m / 100m", 30, jpFont, new Vector2(0.5f,1), new Vector2(0.5f,1), new Vector2(0.5f,1), new Vector2(280,40), new Vector2(0,-110));
        ...
        var clearPanel = CreatePanel(canvasObj.transform, "ClearPanel", new Color(0.2f,0.5f,0.8f,0.9f));
        CT(clearPanel.transform, "ClearTitle", "ゴール！", 52, jpFont, new Vector2(0.5f,0.75f), new Vector2(0.5f,0.75f), new Vector2(0.5f,0.5f), new Vector2(600,100), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        var clearScoreText = CT(clearPanel.transform, "ClearScore", "Score: 0", 40, jpFont, new Vector2(0.5f,0.55f), new Vector2(0.5f,0.55f), new Vector2(0.5f,0.5f), new Vector2(400,60), Vector2.zero);
        clearScoreText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        var clearRetryBtn = CB(clearPanel.transform, "RetryButton", "もう一度", 32, jpFont, new Vector2(0.5f,0.25f), new Vector2(0.5f,0.25f), new Vector2(0.5f,0.5f), new Vector2(220,65), Vector2.zero, new Color(0.2f,0.5f,0.3f));
        clearPanel.SetActive(false);

        var goPanel = CreatePanel(canvasObj.transform, "GameOverPanel", new Color(0.3f,0.1f,0f,0.9f));
        CT(goPanel.transform, "GOTitle", "ゲームオーバー", 52, jpFont, new Vector2(0.5f,0.75f), new Vector2(0.5f,0.75f), new Vector2(0.5f,0.5f), new Vector2(600,100), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        var goScoreText = CT(goPanel.transform, "GOScore", "Score: 0", 40, jpFont, new Vector2(0.5f,0.55f), new Vector2(0.5f,0.55f), new Vector2(0.5f,0.5f), new Vector2(400,60), Vector2.zero);
        ...
        var uiObj = new GameObject("FlyBirdUI"); uiObj.transform.SetParent(gmObj.transform);
        var ui = uiObj.AddComponent<FlyBirdUI>();
        ...
        gmSO.FindProperty("_manager").objectReferenceValue = fm;
        gmSO.FindProperty("_ui").objectReferenceValue = ui;
        gmSO.FindProperty("_goalDistance").floatValue = 100f;
        ...
        string scenePath = "Assets/Scenes/038_FlyBird.unity";
        EditorSceneManager.SaveScene(scene, scenePath); AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup038] FlyBird シーンを作成しました: " + scenePath);
    }
