using UnityEngine;using UnityEngine.UI;using UnityEditor;using UnityEditor.SceneManagement;using UnityEngine.InputSystem.UI;using TMPro;using Game050_BubbleSort;
public static class Setup050_BubbleSort
{
    [MenuItem("Assets/Setup/050 BubbleSort")]
    public static void CreateScene()
    {
        if(EditorApplication.isPlaying){Debug.LogError("[Setup050] Play モード中は実行できません。");return;}
        ...
        var camera=Object.FindFirstObjectByType<Camera>();if(camera!=null){camera.backgroundColor=new Color(0.1f,0.12f,0.2f,1f);camera.orthographic=true;camera.orthographicSize=5f;}

        var gmObj=new GameObject("GameManager");var gm=gmObj.AddComponent<BubbleSortGameManager>();
        var boardObj=new GameObject("SortBoard");boardObj.transform.SetParent(gmObj.transform);var sm=boardObj.AddComponent<BubbleSortManager>();
        ...
        var hint=CT(canvasObj.transform,"HintText","バブルをタップして同色に仕分け!",18,jpFont,new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(450,35),new Vector2(0,20));
        ...
        var uiObj=new GameObject("BubbleSortUI");var bsUI=uiObj.AddComponent<BubbleSortUI>();
        ...
        menuBtn.AddComponent<BackToMenuButton>();var gmSO=new SerializedObject(gm);gmSO.FindProperty("_sortManager").objectReferenceValue=sm;gmSO.FindProperty("_ui").objectReferenceValue=bsUI;gmSO.ApplyModifiedProperties();
        ...
        string scenePath="Assets/Scenes/050_BubbleSort.unity";EditorSceneManager.SaveScene(scene,scenePath);AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup050_BubbleSort] シーンを作成しました: "+scenePath);
    }
