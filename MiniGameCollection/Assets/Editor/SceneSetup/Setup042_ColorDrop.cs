...
using TMPro;
using System.IO;
using Game042_ColorDrop;

public static class Setup042_ColorDrop
{
    [MenuItem("Assets/Setup/042 ColorDrop")]
    public static void CreateScene()
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup042] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game042_ColorDrop/";

        var camera = Object.FindFirstObjectByType<Camera>();
        if (camera != null) { camera.backgroundColor = new Color(0.12f, 0.12f, 0.24f); camera.orthographic = true; camera.orthographicSize = 5.5f; }

        Sprite bgSprite = LoadSprite(sp + "background.png");
        Sprite dR = LoadSprite(sp + "drop_red.png"); Sprite dB = LoadSprite(sp + "drop_blue.png");
        Sprite dG = LoadSprite(sp + "drop_green.png"); Sprite dY = LoadSprite(sp + "drop_yellow.png");
        Sprite bR = LoadSprite(sp + "bucket_red.png"); Sprite bB = LoadSprite(sp + "bucket_blue.png");
        Sprite bG = LoadSprite(sp + "bucket_green.png"); Sprite bY = LoadSprite(sp + "bucket_yellow.png");
        ...
        var gmObj = new GameObject("GameManager"); var gm = gmObj.AddComponent<ColorDropGameManager>();
        var cmObj = new GameObject("ColorDropManager"); cmObj.transform.SetParent(gmObj.transform);
        var cm = cmObj.AddComponent<ColorDropManager>();
        var cmSO = new SerializedObject(cm);
        cmSO.FindProperty("_gameManager").objectReferenceValue = gm;
        cmSO.FindProperty("_dropRed").objectReferenceValue = dR;
        cmSO.FindProperty("_dropBlue").objectReferenceValue = dB;
        cmSO.FindProperty("_dropGreen").objectReferenceValue = dG;
        cmSO.FindProperty("_dropYellow").objectReferenceValue = dY;
        cmSO.FindProperty("_bucketRed").objectReferenceValue = bR;
        cmSO.FindProperty("_bucketBlue").objectReferenceValue = bB;
        cmSO.FindProperty("_bucketGreen").objectReferenceValue = bG;
        cmSO.FindProperty("_bucketYellow").objectReferenceValue = bY;
        ...
        var scoreText = CT(canvasObj.transform, "ScoreText", "Score: 0", 36, jpFont, new Vector2(0,1), new Vector2(0,1), new Vector2(0,1), new Vector2(250,50), new Vector2(20,-20));
        scoreText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Left;
        var missText = CT(canvasObj.transform, "MissText", "Miss: 0/3", 36, jpFont, new Vector2(1,1), new Vector2(1,1), new Vector2(1,1), new Vector2(220,50), new Vector2(-20,-20));
        ...
        var clearPanel = CreatePanel(canvasObj.transform, "ClearPanel", new Color(0.1f,0.2f,0.4f,0.9f));
        CT(clearPanel.transform, "CT", "全ドロップ完了！", 48, jpFont, new Vector2(0.5f,0.75f), new Vector2(0.5f,0.75f), new Vector2(0.5f,0.5f), new Vector2(600,100), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        var clearScoreText = CT(clearPanel.transform, "CS", "Score: 0", 40, jpFont, new Vector2(0.5f,0.55f), new Vector2(0.5f,0.55f), new Vector2(0.5f,0.5f), new Vector2(400,60), Vector2.zero);
        ...
        var goPanel = CreatePanel(canvasObj.transform, "GameOverPanel", new Color(0.3f,0.05f,0f,0.9f));
        CT(goPanel.transform, "GT", "ゲームオーバー", 52, jpFont, new Vector2(0.5f,0.75f), new Vector2(0.5f,0.75f), new Vector2(0.5f,0.5f), new Vector2(600,100), Vector2.zero).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        var goScoreText = CT(goPanel.transform, "GS", "Score: 0", 40, jpFont, new Vector2(0.5f,0.55f), new Vector2(0.5f,0.55f), new Vector2(0.5f,0.5f), new Vector2(400,60), Vector2.zero);
        ...
        var uiObj = new GameObject("ColorDropUI"); uiObj.transform.SetParent(gmObj.transform);
        var ui = uiObj.AddComponent<ColorDropUI>();
        ...
        gmSO.FindProperty("_totalDrops").intValue = 20;
        ...
        string scenePath = "Assets/Scenes/042_ColorDrop.unity";
        EditorSceneManager.SaveScene(scene, scenePath); AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup042] ColorDrop シーンを作成しました: " + scenePath);
    }
