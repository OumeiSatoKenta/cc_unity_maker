using UnityEngine;using UnityEngine.UI;using UnityEditor;using UnityEditor.SceneManagement;using UnityEngine.InputSystem.UI;using TMPro;using Game042_ColorDrop;
public static class Setup042_ColorDrop
{
    [MenuItem("Assets/Setup/042 ColorDrop")]
    public static void CreateScene()
    {
        if(EditorApplication.isPlaying){Debug.LogError("[Setup042_ColorDrop] Play モード中は実行できません。");return;}
        ...
        var camera=Object.FindFirstObjectByType<Camera>();if(camera!=null){camera.backgroundColor=new Color(0.08f,0.1f,0.18f,1f);camera.orthographic=true;camera.orthographicSize=5f;}
        string whiteTexPath="Assets/Scripts/Game042_ColorDrop/WhiteSquare.png";
        ...
        var boardObj=new GameObject("DropBoard");boardObj.transform.SetParent(gmObj.transform);var dm=boardObj.AddComponent<DropManager>();
        var dmSO=new SerializedObject(dm);dmSO.FindProperty("_gameManager").objectReferenceValue=gm;dmSO.ApplyModifiedProperties();
        ...
        var hint=CT(canvasObj.transform,"HintText","雨粒を同色のバケツへ!",20,jpFont,new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(0.5f,0),new Vector2(400,40),new Vector2(0,20));
        ...
        var menuBtn=CB(canvasObj.transform,"MenuButton","メニューへ戻る",24,jpFont,new Vector2(0,0),new Vector2(0,0),new Vector2(0,0),new Vector2(240,50),new Vector2(20,20),new Color(0.3f,0.3f,0.4f,0.9f));
        ...
        var finalText=CT(goPanel.transform,"FinalScoreText","スコア: 0",36,jpFont,new Vector2(0.5f,0.45f),new Vector2(0.5f,0.45f),new Vector2(0.5f,0.5f),new Vector2(300,50),Vector2.zero);
        ...
        var uiObj=new GameObject("ColorDropUI");var cdUI=uiObj.AddComponent<ColorDropUI>();
        ...
        menuBtn.AddComponent<BackToMenuButton>();var gmSO=new SerializedObject(gm);gmSO.FindProperty("_dropManager").objectReferenceValue=dm;gmSO.FindProperty("_ui").objectReferenceValue=cdUI;gmSO.ApplyModifiedProperties();
        ...
        string scenePath="Assets/Scenes/042_ColorDrop.unity";EditorSceneManager.SaveScene(scene,scenePath);AddSceneToBuildSettings(scenePath);
        Debug.Log("[Setup042_ColorDrop] ColorDrop シーンを作成しました: "+scenePath);
    }
