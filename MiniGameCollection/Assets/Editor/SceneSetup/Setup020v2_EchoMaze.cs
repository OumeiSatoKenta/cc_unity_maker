...
using TMPro;
using System.IO;
using Game020v2_EchoMaze;

public static class Setup020v2_EchoMaze
{
    [MenuItem("Assets/Setup/020v2 EchoMaze")]
    public static void CreateScene()
    {
        if (EditorApplication.isPlaying) { Debug.LogError("[Setup020v2] Play モード中は実行できません。"); return; }
        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        var jpFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/Fonts/NotoSansJP-Regular SDF.asset");
        string sp = "Assets/Resources/Sprites/Game020v2_EchoMaze/";
        ...
        string[] spritePaths = { sp+"Background.png", sp+"Player.png", sp+"Cell.png", sp+"Goal.png", sp+"Portal.png" };
        ...
        Sprite spCell   = LoadSprite(sp + "Cell.png");
        Sprite spGoal   = LoadSprite(sp + "Goal.png");
        Sprite spPortal = LoadSprite(sp + "Portal.png");
        ...
        var gm = gmObj.AddComponent<EchoMazeGameManager>();
        ...
        // MazeController (child)
        var mcObj = new GameObject("MazeController");
        mcObj.transform.SetParent(gmObj.transform);
        var mc = mcObj.AddComponent<MazeController>();
        ...
        // Move limit (top-center)
        var moveLimitText = CT(canvasObj.transform, "MoveLimitText", "残り 30/30", 32, jpFont,
            new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f),
            new Vector2(240, 50), new Vector2(0, -30));
        moveLimitText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        moveLimitText.GetComponent<TextMeshProUGUI>().color = new Color(0.7f, 0.9f, 1f);

        // Floor indicator (2nd row, center) - for Stage 5
        var floorText = CT(canvasObj.transform, "FloorIndicatorText", "1階", 28, jpFont,
            new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f),
            new Vector2(120, 44), new Vector2(0, -85));
        floorText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        floorText.GetComponent<TextMeshProUGUI>().color = new Color(0.8f, 0.6f, 1f);

        // ---- Echo Indicator Panel (center-bottom area, above buttons) ----
        var echoPanel = CreatePanel(canvasObj.transform, "EchoIndicatorPanel", new Color(0.05f, 0.08f, 0.18f, 0.85f),
            new Vector2(340, 220));
        var epRt = echoPanel.GetComponent<RectTransform>();
        epRt.anchorMin = new Vector2(0.5f, 0f);
        epRt.anchorMax = new Vector2(0.5f, 0f);
        epRt.pivot = new Vector2(0.5f, 0f);
        epRt.anchoredPosition = new Vector2(0, 155);

        // Echo label
        var echoLabel = CT(echoPanel.transform, "EchoLabel", "エコー", 26, jpFont,
            new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f),
            new Vector2(300, 38), new Vector2(0, -8));
        echoLabel.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        echoLabel.GetComponent<TextMeshProUGUI>().color = new Color(0.6f, 0.8f, 1f);

        // Disturb warning
        var disturbText = CT(echoPanel.transform, "EchoDisturbText", "⚠ 撹乱", 24, jpFont,
            new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f),
            new Vector2(200, 36), new Vector2(0, -48));
        disturbText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        disturbText.GetComponent<TextMeshProUGUI>().color = new Color(1f, 0.5f, 0.3f);
        disturbText.SetActive(false);

        // Echo bars: North, South, West, East
        var northBar = CreateEchoBar(echoPanel.transform, "EchoNorthBar", new Vector2(0.5f, 0.75f), new Vector2(120, 22));
        var southBar = CreateEchoBar(echoPanel.transform, "EchoSouthBar", new Vector2(0.5f, 0.25f), new Vector2(120, 22));
        var westBar  = CreateEchoBar(echoPanel.transform, "EchoWestBar",  new Vector2(0.25f, 0.5f), new Vector2(22, 120));
        var eastBar  = CreateEchoBar(echoPanel.transform, "EchoEastBar",  new Vector2(0.75f, 0.5f), new Vector2(22, 120));
        // Direction labels
        CT(echoPanel.transform, "NLabel", "N", 22, jpFont, new Vector2(0.5f, 0.9f), new Vector2(0.5f, 0.9f), new Vector2(0.5f, 0.5f), new Vector2(40, 30), new Vector2(0, 0)).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        CT(echoPanel.transform, "SLabel", "S", 22, jpFont, new Vector2(0.5f, 0.1f), new Vector2(0.5f, 0.1f), new Vector2(0.5f, 0.5f), new Vector2(40, 30), new Vector2(0, 0)).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        CT(echoPanel.transform, "WLabel", "W", 22, jpFont, new Vector2(0.1f, 0.5f), new Vector2(0.1f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(40, 30), new Vector2(0, 0)).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        CT(echoPanel.transform, "ELabel", "E", 22, jpFont, new Vector2(0.9f, 0.5f), new Vector2(0.9f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(40, 30), new Vector2(0, 0)).GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;

        // ---- Map Panel ----
        var mapPanel = CreatePanel(canvasObj.transform, "MapPanel", new Color(0.02f, 0.05f, 0.1f, 0.96f),
            new Vector2(500, 500));
        var mapText = CT(mapPanel.transform, "MapText", "", 22, jpFont,
            new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
            new Vector2(480, 480), new Vector2(0, 0));
        mapText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.TopLeft;
        mapText.GetComponent<TextMeshProUGUI>().color = new Color(0.7f, 1f, 0.8f);
        mapPanel.SetActive(false);

        // ---- Direction Buttons ----
        // Arranged in D-pad layout above the menu button
        ...
        var upBtn = CB(canvasObj.transform, "UpButton", "↑", jpFont,
            ...
            new Vector2(100, 80), new Vector2(0, 385), new Color(0.15f, 0.3f, 0.6f));
        // Down button (center-bottom)
        var downBtn = CB(canvasObj.transform, "DownButton", "↓", jpFont,
            new Vector2(0.35f, 0f), new Vector2(0.35f, 0f), new Vector2(0.5f, 0f),
            new Vector2(100, 80), new Vector2(0, 225), new Color(0.15f, 0.3f, 0.6f));
        // Left button
        var leftBtn = CB(canvasObj.transform, "LeftButton", "←", jpFont,
            new Vector2(0.35f, 0f), new Vector2(0.35f, 0f), new Vector2(0.5f, 0f),
            new Vector2(100, 80), new Vector2(-110, 305), new Color(0.15f, 0.3f, 0.6f));
        // Right button
        var rightBtn = CB(canvasObj.transform, "RightButton", "→", jpFont,
            new Vector2(0.35f, 0f), new Vector2(0.35f, 0f), new Vector2(0.5f, 0f),
            new Vector2(100, 80), new Vector2(110, 305), new Color(0.15f, 0.3f, 0.6f));
        // Echo center button
        var echoBtn = CB(canvasObj.transform, "EchoButton", "echo", jpFont,
            new Vector2(0.35f, 0f), new Vector2(0.35f, 0f), new Vector2(0.5f, 0f),
            new Vector2(100, 80), new Vector2(0, 305), new Color(0.1f, 0.2f, 0.5f));

        // Map button (right side)
        var mapBtn = CB(canvasObj.transform, "MapButton", "MAP", jpFont,
            new Vector2(1f, 0f), new Vector2(1f, 0f), new Vector2(1f, 0f),
            new Vector2(110, 70), new Vector2(-10, 320), new Color(0.2f, 0.4f, 0.3f));

        // Bottom buttons
        var menuBtn = CB(canvasObj.transform, "MenuButton", "メニューへ戻る", jpFont,
            new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0.5f, 0f),
            new Vector2(280, 65), new Vector2(-120, 80), new Color(0.25f, 0.25f, 0.4f));

        var retryBtn = CB(canvasObj.transform, "RetryButton", "リスタート", jpFont,
            new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0.5f, 0f),
            new Vector2(200, 65), new Vector2(100, 80), new Color(0.3f, 0.5f, 0.25f));

        var reShowBtn = CB(canvasObj.transform, "ReShowInstructionBtn", "?", jpFont,
            new Vector2(1f, 0f), new Vector2(1f, 0f), new Vector2(1f, 0f),
            new Vector2(75, 75), new Vector2(-10, 80), new Color(0.2f, 0.4f, 0.7f));
        ...
        var scScore = CT(scPanel.transform, "StageClearScoreText", "+0pt", 46, jpFont,
            new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
            new Vector2(600, 65), new Vector2(0, 60));
        ...
        var scBonus = CT(scPanel.transform, "StageClearBonusText", "", 28, jpFont,
            new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
            new Vector2(680, 60), new Vector2(0, -10));
        scBonus.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        scBonus.GetComponent<TextMeshProUGUI>().color = new Color(0.5f, 1f, 0.7f);
        var nextBtn = CB(scPanel.transform, "NextStageButton", "次のステージへ", jpFont,
            new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0.5f, 0f),
            new Vector2(320, 70), new Vector2(0, 35), new Color(0.2f, 0.55f, 0.9f));
        ...
        var goSubText = CT(goPanel.transform, "GameOverSub", "移動回数が尽きました", 34, jpFont,
            new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
            new Vector2(600, 55), new Vector2(0, 30));
        goSubText.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;
        goSubText.GetComponent<TextMeshProUGUI>().color = new Color(1f, 0.6f, 0.6f);
        var goRetryBtn = CB(goPanel.transform, "RetryButton", "もう一度", jpFont,
            new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0.5f, 0f),
            new Vector2(280, 70), new Vector2(0, 70), new Color(0.6f, 0.35f, 0.15f));
        var goMenuBtn = CB(goPanel.transform, "GOMenuButton", "メニューへ戻る", jpFont,
            new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0.5f, 0f),
            new Vector2(280, 60), new Vector2(0, 10), new Color(0.25f, 0.25f, 0.4f));
        ...
        var gcMenuBtn = CB(gcPanel.transform, "GCMenuButton", "メニューへ戻る", jpFont,
            new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0.5f, 0f),
            new Vector2(300, 65), new Vector2(0, 30), new Color(0.3f, 0.3f, 0.5f));
        ...
        var ipTitle = CT(ipPanel.transform, "IPTitle", "EchoMaze", 60, jpFont,
            new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
        ...
        var ipStartBtn = CB(ipPanel.transform, "IPStartButton", "はじめる", jpFont,
            new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
            new Vector2(300, 75), new Vector2(0, -200), new Color(0.2f, 0.5f, 0.85f));
        ...
        // ---- EchoMazeUI ----
        var uiObj = new GameObject("EchoMazeUI");
        uiObj.transform.SetParent(gmObj.transform);
        var echoUI = uiObj.AddComponent<EchoMazeUI>();
        var uiSO = new SerializedObject(echoUI);
        uiSO.FindProperty("_stageText").objectReferenceValue = stageText.GetComponent<TextMeshProUGUI>();
        uiSO.FindProperty("_scoreText").objectReferenceValue = scoreText.GetComponent<TextMeshProUGUI>();
        uiSO.FindProperty("_moveLimitText").objectReferenceValue = moveLimitText.GetComponent<TextMeshProUGUI>();
        uiSO.FindProperty("_floorIndicatorText").objectReferenceValue = floorText.GetComponent<TextMeshProUGUI>();
        uiSO.FindProperty("_echoNorthBar").objectReferenceValue = northBar.GetComponent<Image>();
        uiSO.FindProperty("_echoSouthBar").objectReferenceValue = southBar.GetComponent<Image>();
        uiSO.FindProperty("_echoWestBar").objectReferenceValue = westBar.GetComponent<Image>();
        uiSO.FindProperty("_echoEastBar").objectReferenceValue = eastBar.GetComponent<Image>();
        uiSO.FindProperty("_echoDisturbText").objectReferenceValue = disturbText.GetComponent<TextMeshProUGUI>();
        uiSO.FindProperty("_mapPanel").objectReferenceValue = mapPanel;
        uiSO.FindProperty("_mapText").objectReferenceValue = mapText.GetComponent<TextMeshProUGUI>();
        ...
        mcSO.FindProperty("_gameManager").objectReferenceValue = gm;
        mcSO.FindProperty("_ui").objectReferenceValue = echoUI;
        mcSO.FindProperty("_playerSprite").objectReferenceValue = spPlayer;
        mcSO.FindProperty("_cellSprite").objectReferenceValue = spCell;
        mcSO.FindProperty("_goalSprite").objectReferenceValue = spGoal;
        mcSO.FindProperty("_portalSprite").objectReferenceValue = spPortal;
        ...
        gmSO.FindProperty("_mazeController").objectReferenceValue = mc;
        gmSO.FindProperty("_ui").objectReferenceValue = echoUI;
        gmSO.ApplyModifiedProperties();

        // ---- Button Events ----
        AddButtonOnClick(upBtn.GetComponent<Button>(),    mc, "OnUpButton");
        AddButtonOnClick(downBtn.GetComponent<Button>(),  mc, "OnDownButton");
        AddButtonOnClick(leftBtn.GetComponent<Button>(),  mc, "OnLeftButton");
        AddButtonOnClick(rightBtn.GetComponent<Button>(), mc, "OnRightButton");
        AddButtonOnClick(echoBtn.GetComponent<Button>(),  mc, "EmitEcho");
        AddButtonOnClick(mapBtn.GetComponent<Button>(),   mc, "ToggleMap");
        ...
        // ---- Save Scene ----
        string scenePath = "Assets/Scenes/020v2_EchoMaze.unity";
        EditorSceneManager.SaveScene(scene, scenePath);
        AddSceneToBuildSettings(scenePath);
        AssetDatabase.Refresh();
        Debug.Log("[Setup020v2] Scene created: " + scenePath);
    }

    static GameObject CreateEchoBar(Transform parent, string name, Vector2 anchor, Vector2 size)
    {
        ...
        bgImg.color = new Color(0.1f, 0.15f, 0.3f, 0.8f);
        // Fill
        ...
        fillImg.fillMethod = size.x > size.y ? Image.FillMethod.Horizontal : Image.FillMethod.Vertical;
        fillImg.fillOrigin = 0;
        fillImg.fillAmount = 0.5f;
        return fillGo; // return the fill image object
    }
        ...
        Vector2 anchorMin, Vector2 anchorMax, Vector2 pivot, Vector2 sizeDelta, Vector2 anchoredPos, Color bgColor)
    {
        ...
        var colors = btn.colors; colors.highlightedColor = bgColor * 1.3f; colors.pressedColor = bgColor * 0.7f; btn.colors = colors;
    ...
    static Sprite LoadSprite(string path)
    {
        ...
        if (sprite == null)
        {
            ...
            else Debug.LogWarning($"[Setup020v2] Sprite not found: {path}");
        }
        return sprite;
    }
