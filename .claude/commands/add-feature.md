...
2. 直近で実装済みのゲームがあれば、そのコードも参照する。
3. 既存の実装パターン、命名規則、コンポーネントの利用方法を確認する。
...
2. **全ての重要な指摘が解消されたことを確認してから、ステップ4に進むこと。**
...
`implementation-validator` サブエージェントを起動して品質を検証する:
- 対象: 今回実装した全ファイル
- 観点: コーディング規約、エラーハンドリング、既存パターンとの整合性

### 5-2. 3軸コードレビューとフィードバックループ

以下の3つのサブエージェントを**並列で**起動する:
...
2. **総合評価が B 以上**、かつ `[必須]` が0件になったら、次のステップへ進む。
...
- ステップ5-2: 3軸コードレビューで総合評価 B 以上、`[必須]` が0件。
