...
対象ゲームの `id`, `title`, `category`, `sceneName`, `description`, `collection` を特定する。

### 1-0. Classic / Remake モード判定（必須）

GameRegistry.json を確認し、対象IDのエントリーを以下のルールで選択する:

**エントリー選択ルール（優先順位順）:**

1. 対象IDに `collection: "remake"` のエントリーが存在する場合 → **remakeエントリーを使用**（`implemented: true` でも確認不要、そのまま進む）
2. 対象IDに `collection: "classic"` のみ存在し `implemented: false` の場合 → classicエントリーを使用
3. 対象IDに `collection: "classic"` のみ存在し `implemented: true` の場合 → **警告を出してユーザーに確認を求める**（remakeエントリーが存在しないため）

| collection | 実装モード | スクリプトディレクトリ | シーン名 | Setup メニュー |
|-----------|----------|---------------------|---------|--------------|
| `classic`  | v1 シンプル実装（StageManager/InstructionPanel 不使用） | `Game[ID]_[Title]/` | `[ID]_[Title]` | `Assets/Setup/[ID] [Title]` |
| `remake`   | v2 フル実装（5ステージ・StageManager・InstructionPanel） | `Game[ID]v2_[Title]/` | `[ID]v2_[Title]` | `Assets/Setup/[ID]v2 [Title]` |

- `classic` の場合: **ステップ4以降でStageManager・InstructionPanelを使わない**シンプルな実装を行う。SceneSetup は `Setup[ID]_[Title].cs`。
- `remake` の場合: 通常通り v2 フル実装を行う。スクリプトはすべて `namespace Game[ID]v2_[Title]`、SceneSetup は `Setup[ID]v2_[Title].cs`。

> **注意**: 同一IDにclassicとremakeが両方存在する場合、remakeを優先して実装対象とする。
...
- [ ] **レスポンシブ配置**: ワールド座標のオブジェクト配置が `Camera.main.orthographicSize` から動的に計算されているか（固定座標のハードコーディングは禁止）。Canvas UIボタンとゲーム要素が重ならないよう、下部に2.5u以上のマージンが確保されているか
...
- **レスポンシブ配置（必須）:**
  - ワールド座標のオブジェクト配置は **カメラの `orthographicSize` とアスペクト比から動的に計算** する。固定座標のハードコーディングは禁止
  - 画面を3領域に分割して設計する:
    - **上部マージン（HUD用）**: `camSize - 1.2` 以上をHUD領域として確保
    - **中央（ゲーム領域）**: 動的にサイズ計算（利用可能高さ÷要素数でセルサイズを決定）
    - **下部マージン（UI用）**: 下端から `2.5〜3.0` ユニットをCanvas UIボタン領域として確保
  - セルサイズやグリッドサイズが変わるゲーム（ステージ進行等）では、`SetupStage()` のたびに再計算すること
  - 参考実装パターン:
    ```csharp
...
- **UIレスポンシブ配置ルール（必須）:**
  - Canvas のボタンやテキストは **ワールド座標のゲーム要素と重ならない** よう配置する
  - ボタン配置の3レイヤー構成:
    - **最下部（Y=10〜20）**: メニューへ戻るボタン（常時表示）
    - **下段（Y=65〜80）**: ゲーム操作ボタン（発射・リセット等）を横並び
    - **上部（Y=-15〜-60）**: HUDテキスト（ステージ・スコア）
  - 操作ボタンが複数ある場合は **横に並べる**（縦に積まない）。左右のアンカーで分散配置
  - ゲーム領域のワールド座標と Canvas UI が重なる場合は、ゲーム側の `bottomMargin` を増やして対応する
...
## 参考: Classic / Remake パターン

### Remake (v2) パターン参照先
- `MiniGameCollection/Assets/Scripts/Game002v2_MirrorMaze/`
- `MiniGameCollection/Assets/Editor/SceneSetup/Setup002v2_MirrorMaze.cs`

### Classic (v1) パターン参照先
- `MiniGameCollection/Assets/Scripts/Game002_MirrorMaze/` （MazeManager, MirrorController, MirrorMazeGameManager, MirrorMazeUI）
- `MiniGameCollection/Assets/Editor/SceneSetup/Setup002_MirrorMaze.cs`

> **重要**: Classic 実装時は StageManager・InstructionPanel を**一切使わない**。シンプルな3ステージループ構成（`StageCount = 3`）で実装する。
